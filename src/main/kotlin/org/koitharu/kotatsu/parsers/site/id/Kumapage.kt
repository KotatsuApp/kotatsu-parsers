	...
	override suspend fun getListPage(page: Int, order: SortOrder, filter: MangaListFilter): List<Manga> {
		return when {
			!filter.query.isNullOrEmpty() -> {
				...
				val headers = headersOf(HttpHeaders.ContentType, ContentType.Application.FormUrlEncoded.toString())
				val response =
					webClient.httpPost(Url(url), payload = "view=1&keyword=${filter.query}&genre=$genre", headers)
						.parseHtml()
			...
			filter.tags.isNotEmpty() -> {
				...
				val response = webClient.httpGet(Url(url)).parseHtml()

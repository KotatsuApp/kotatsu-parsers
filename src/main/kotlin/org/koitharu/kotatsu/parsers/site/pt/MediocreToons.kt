...
internal class MediocreToons(context: MangaLoaderContext) : PagedMangaParser(
	...
	override suspend fun getListPage(page: Int, order: SortOrder, filter: MangaListFilter): List<Manga> {
		val url = when {
			!filter.query.isNullOrEmpty() || filter.tags.isNotEmpty() ||
				filter.states.isNotEmpty() || filter.types.isNotEmpty() -> buildSearchUrl(page, filter)

			else -> {
				"$apiUrl/obras".toHttpUrl().newBuilder()
	...
	override suspend fun getDetails(manga: Manga): Manga {
		...
		val description = response.optString("descricao")
			.replace(Regex("</?[^>]+>"), "")
			.replace("\\/", "/")
			.replace(Regex("\\s+"), " ")
			.trim()

...
internal abstract class LibSocialParser(
	...
	override suspend fun getListPage(page: Int, order: SortOrder, filter: MangaListFilter): List<Manga> {
		val urlBuilder = URLBuilder().apply {
			...
			host = "api.lib.social"
			appendPathSegments("api", "manga")
			parameters.append("site_id[]", siteId.toString())
			parameters.append("fields[]", "rate")
			parameters.append("fields[]", "rate_avg")
			parameters.append("page", page.toString())
		}
		for (state in filter.states) {
			urlBuilder.parameters.append("status[]", statesMap.keyOf(state).toString())
		}
		for (tag in filter.tags) {
			urlBuilder.parameters.append("${tag.typeKey()}[]", tag.key.drop(1))
		}
		for (tag in filter.tagsExclude) {
			urlBuilder.parameters.append("${tag.typeKey()}_exclude[]", tag.key.drop(1))
		}
		if (!filter.query.isNullOrEmpty()) {
			urlBuilder.parameters.append("q", filter.query)
		}
	...
	override suspend fun getDetails(manga: Manga): Manga = coroutineScope {
		...
		val url = URLBuilder().apply {
			protocol = URLProtocol.HTTPS
			host = "api.lib.social"
			appendPathSegments("api", "manga", manga.url)
			parameters.append("fields[]", "summary")
			parameters.append("fields[]", "genres")
			parameters.append("fields[]", "tags")
			parameters.append("fields[]", "authors")
		}.build()
	...
	override suspend fun getRelatedManga(seed: Manga): List<Manga> {
		val json = webClient.httpGet(
			URLBuilder().apply {
				protocol = URLProtocol.HTTPS
				host = "api.lib.social"
				appendPathSegments("api", "manga", seed.url, "similar")
			}.build(),
	...
	private suspend fun fetchChapters(manga: Manga): List<MangaChapter> {
		val url = URLBuilder().apply {
			protocol = URLProtocol.HTTPS
			host = "api.lib.social"
			appendPathSegments("api", "manga", manga.url, "chapters")
		}.build()
	...
	private suspend fun fetchServers(): ScatterMap<String, String> {
		val json = webClient.httpGet(
			URLBuilder().apply {
				protocol = URLProtocol.HTTPS
				host = "api.lib.social"
				appendPathSegments("api", "constants")
				parameters.append("fields[]", "imageServers")
			}.build(),
